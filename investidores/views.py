...
def realizar_proposta(request, id):
    ...
    if valuation < (int(empresa.valuation / 2)):
        messages.add_message(request, constants.WARNING, f'Seu valuation proposto RS{valuation} e deve ser no mínimo {empresa.valuation}')
        return redirect(f'/investidores/ver_empresa/{id}')
    ...
    pi.save()
    return redirect(f'/investidores/assinar_contrato/{pi.id}') #Vai dar erro
